...
async function fetchFromGitHub(path = '') {
  ...
  try {
    const response = await fetch(url, {
      ...
      // Next.js 캐싱 설정 - 24시간마다 재검증 (API 호출 대폭 감소)
...
export async function getAllDocuments() {
  try {
    ...
    for (const category of categories) {
      ...
      // 메타데이터 없이 기본 정보만 포함 (API 호출 최소화)
      for (const doc of documents) {
        allDocuments.push({
          ...doc,
          category: category.name,
          title: doc.name.replace('.md', '').replace(/-/g, ' '),
          description: '클릭해서 내용을 확인하세요',
          tags: [],
          status: 'published',
          author: 'DemianDev',
...
export async function getDocumentBySlug(slug) {
  try {
    ...
    // 각 카테고리에서 파일 목록만 먼저 확인
    for (const category of categories) {
      const documents = await getDocumentsByCategory(category.name);
      const foundDoc = documents.find(doc => doc.slug === slug);
      
      if (foundDoc) {
        console.log(`문서 찾음: ${category.name}/${slug}.md`);
        // 찾은 경우에만 실제 내용 가져오기
        const fullContent = await getDocumentContent(category.name, slug);
        return {
          ...fullContent,
          category: category.name
        };
